# coding=utf-8
import numpy as np
from Integration.common_func import *
...
def courier_intg(dc_cr):  # 快递员的整合
    ...
    dc_cr['快递员当月应发放提成金额฿'] = dc_cr[['快递员当月应发放提成金额฿', 'buddy奖励','人效补贴',
                                    '驾照补贴', '支援补贴', 'wongnai补贴','芭提雅&普吉岛补贴','场地补助']].apply(no_deduct, axis=1)  # 不减扣项；
    # dc_cr['快递员当月应发放提成金额฿'] = dc_cr[['快递员当月应发放提成金额฿', '多发补扣']].apply(max_zero, axis=1)
    return dc_cr


def officer_intg(dc_of):  # 仓管员的整合
    # idxs = (np.where(dc_of.columns == '仓管员奖金池合计'))[0][0]
    ...
    # dc_of['个人罚款合计调整'] = dc_of['个人罚款合计'] * dc_of['揽件人效系数']
    dc_of['个人扣款合计'] = dc_of['个人罚款合计'] + dc_of['扣话费'] + dc_of['补扣工资'] + dc_of['低效外协处罚']
                      # + dc_of['虚假扫描罚款']
    ...
    dc_of['本月应发放提成฿'] = dc_of[['本月应发放提成฿', 'buddy奖励','支援补贴','芭提雅&普吉岛补贴','场地补助']].apply(no_deduct, axis=1)  # 不减扣项；
...
def supervisor_intg(sup):  # 主管的整合
    ...
    sup['本月应发放提成฿'] = sup[['本月应发放提成฿',  'buddy奖励', '场地补助', '支援补贴','芭提雅&普吉岛补贴']].apply(no_deduct, axis=1)  # 不减扣项；
